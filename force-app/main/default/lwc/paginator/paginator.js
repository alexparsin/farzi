...
export default class Paginator extends LightningElement {
    ...
    @api cmpPageSize;
    @track defaultComPage=false;
    @api comPage=false;

    @track pFive=false;
    @track pTen=false;
    @track pTwentyFive=false;
    @track pFifty=false;
    @track pHundred=false;

    //Called after the component finishes inserting to DOM
    setSelectDefault(value){
        console.log('default value--->'+value);
       if(value==5) this.pFive=true;
       if(value==10) this.pTen=true;
       if(value==25) this.pTwentyFive=true;
       if(value==50) this.pFifty=true;
       if(value==100) this.pHundred=true;

    }

    connectedCallback() {
        console.log('paginator records-->'+JSON.stringify(this.records));
        console.log('Paginator :: connectedCallback ' + this.totalRecords);
        if(this.cmpPageSize){
            this.pageSize=this.cmpPageSize;
            this.setSelectDefault(this.pageSize);
